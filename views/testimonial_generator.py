...
1. \n
[Name] was a student of [School] from {cur_year-3} to {cur_year}.
...
with col3:
    stu_cca = st.selectbox("Select CCA of student:", ("Badminton", "Scouts", "NCC", "Basketball", "Drama", "Band"), index = None, placeholder="Select CCA...")
with col4:
...
if st.button("Generate Testimonial"):
    ...
    if not stu_name or not stu_att or not stu_cca or stu_lead is None:
        ...
        with open(f'{stu_name}.docx', 'rb') as f:
            st.download_button('Download Testimonial as Docx', f, file_name=f'{stu_name}.docx')
        docx_files = [ f for f in os.listdir(os.curdir) if f.endswith(".docx") ]
        for f in docx_files:
